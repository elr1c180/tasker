...
buttons = [
    types.InlineKeyboardButton(text=str(i), callback_data=f"theme_{i}")
    for i in range(1, 8)
]
...
def get_paginated_keyboard(page: int, page_size: int = 3):
    ...
    if start_index > 0:
        navigation_buttons.append(
            types.InlineKeyboardButton(text="⬅️ Назад", callback_data=f"themePage_{page-1}")
        )
    
    if end_index < len(buttons):
        navigation_buttons.append(
            types.InlineKeyboardButton(text="Вперед ➡️", callback_data=f"themePage_{page+1}")
        )
